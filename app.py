...
def main():
    ...
    if st.button("Run Query"):
        ...
        # Additional check for sales quantity without time period
        sales_quantity_exact_pattern = re.compile(r"SUM\s*\(\s*SalesQuantity\s*\)", re.IGNORECASE)
        time_keywords = [
            "yesterday", "today", "last week", "last month", "this week", "this month", 
            "on", "between", "from", "to", "qtd", "quarter", "mtd", "month", "wtd", "week", 
            "ytd", "year", "l7d", "last 7 days", "lw", "previous week", "pw"
        ]
        has_time_keyword = any(time_kw in user_query.lower() for time_kw in time_keywords)
        if sales_quantity_exact_pattern.search(sql_query) and not has_time_keyword:
            st.warning("Please specify a time period for the sales quantity (e.g., 'last week', 'yesterday', 'QTD', 'MTD', 'L7D', etc.).")
            return
